...
from jax import random, numpy as jnp, scipy as jsp

import h5py

import jax
from afqmc import Proagator, Trial
...
# definitely Correct
    ...
    def get_green_func(self, wf1, wf2):
        ...
        inv_ovlp_alpha = jnp.linalg.inv(jnp.einsum('pr, pq -> rq', wf1.tensor_alpha.conj(), wf2.tensor_alpha))

        inv_ovlp_beta = jnp.linalg.inv(jnp.einsum('pr, pq -> rq', wf1.tensor_beta.conj(), wf2.tensor_beta))

        intermediate_alpha = jnp.einsum('jk, kl -> jl', wf2.tensor_alpha, inv_ovlp_alpha)

        green_func_alpha = jnp.einsum('jl, ml -> mj', intermediate_alpha, wf1.tensor_alpha.conj())

        intermediate_beta = jnp.einsum('jk, kl -> jl', wf2.tensor_beta, inv_ovlp_beta)

        green_func_beta = jnp.einsum('jl, ml -> mj', intermediate_beta, wf1.tensor_beta.conj())
...
# definitely Correct
    ...
    def get_overlap(self, wf1, wf2):
        ...
        overlap_matrix_alpha = jnp.einsum('pr, pq -> rq', wf1.tensor_alpha.conj(), wf2.tensor_alpha)

        overlap_matrix_beta = jnp.einsum('pr, pq -> rq', wf1.tensor_beta.conj(), wf2.tensor_beta)
    ...
    def propagate(self, wf, h1e, l_tensor, dt, ds, aux):
        ...
        def helper(wf, h1e, l_tensor, dt, ds, aux, prob):
            ...
            wf.tensor_alpha = C * prob * jnp.einsum('pq, qr -> pr', expA, tempa)

            wf.tensor_beta = C * prob * jnp.einsum('pq, qr -> pr', expA, tempb)
...
# Definitely Correct
    ...
    def met_hasting_sample(self, ham, h1e, l_tensor, dt, ds, key, nsteps=10000, step_size=0.1, burn_in=400):
    ...
    # BEFORE SAMPLING ENUMERATE AUXILARY FIELDS
    ...
    def get_variational_energy(self, x, h1e, v2e, nuc):
       ...
       # self.auxiliary_distribution(h1e, l_tensor_params, dt_params, ds_params, xprime)
        ...
        ax, mags, phases, energies, self.key = self.met_hasting_sample(ham, h1e_params, l_tensor_params, dt_params, ds_params, self.key)
       ...
       # plt.show()
        ...
        #print("Normalized Magnitudes", norm_mags[:5])
    ...
    def gradient(self, x, h1e, v2e, nuc):
        ...
        return np.array(jax.grad(self.get_variational_energy)(x, h1e, v2e, nuc), dtype=np.float64)



    def run(self, tau, max_iter=15000, tol=1e-10, disp=True):
