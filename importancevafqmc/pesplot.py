...
for npy_path in sorted(glob.glob("params/optimal_param-*.npy")):
    ...
    if match:
       ...
       # reblocked_ipie= reblock(ipierun(mol))
        ...
        prop = Propagator(mol, dt=dt, nsteps=nsteps, nwalkers=10**6) # Example parameters
        ...
        energy = jnp.real((energy) / jnp.sum(jnp.real(phase)))
        vafqmcenergy.append(jnp.sum(energy))
        vafqmcerror.append(jnp.std(jnp.real((energy)/jnp.real(phase)))) #This might not be the way to cALCULATE
...
#plt.plot(dist_for_param, vafqmcenergy, label="VAFQMC")
plt.plot(sorted_distances, fcienergies, label="fci", linestyle = '--', color = 'green')
# Labels and legend
plt.plot(sorted_distances, hfenergies, label="hf", linestyle = '--')
