...
class Propagator(object):
    def __init__(self, mol, dt, nsteps, nwalkers=10000, num_chains = 1,
    ...
    def sampler(self, num_warmup, key): 
        ...
        # Vectorize over the keys and initial params
            vectorized_run = jax.vmap(run_single_chain, in_axes=(0, 0))

            # Run the chains in parallel
            samples = vectorized_run(keys, init_params)
    ...
    def objective_func_grad(self, params, key):  
    ...
    def objective_func(self, params):
        ...
        def _objective_func(key):   
            ...
            num = jnp.sum(energies)
            denom = jnp.sum(phases)
            return jnp.real(num / denom) #+ lam *(jnp.maximum(B-jnp.real(jnp.mean(phases)), 0))**2, newkey
        val = _objective_func(self.key)
        self.key, _ = jax.random.split(self.key)
        return val

    def gradient(self, params):
        @jit
        def _gradient(params, key):
            newkey, subkey = jax.random.split(key)
            gradient_fn = lambda p: self.objective_func_grad(p, subkey)
            grad_params = jax.grad(gradient_fn)(params)
            return grad_params, newkey
        grad, self.key = _gradient(params, self.key)
        return grad
        


    def run(self, max_iter=1000, tol=1e-5, disp=True):
        ...
        params = jnp.concatenate([h1e_repeated.flatten(), 
                                  ...
                                  t, s])
        print("energy",self.objective_func(params))
...
if __name__ == "__main__":
    ...
    for _ in times:
       ...
       # prop = JaxPropagator(mol, dt=0.01, total_t=time, nwalkers=100)  
        ...
        prop = Propagator(mol, dt=0.005, nsteps=300, nwalkers=1000)
